...
export async function searchManheimListings(args: { query?: string; vin?: string; limit?: number }) {
  const endpoint = process.env.MANHEIM_API_URL;
  const apiKey = process.env.MANHEIM_API_KEY;
  requireIntegrationConfig('Manheim', endpoint, apiKey);
  if (!endpoint || !apiKey) {
    throw new AppError('Manheim integration is not configured', 500, 'INTEGRATION_CONFIG_ERROR');
  }
  ...
  try {
    ...
    if (!response.ok) {
      throw new AppError(`Manheim request failed with status ${response.status}`, 502, 'INTEGRATION_UPSTREAM_ERROR');
    }

    const payload = await response.json();
    const items: any[] = Array.isArray(payload?.items) ? payload.items : [];
    return items.map((item): AuctionSearchResult => ({
      source: 'MANHEIM',
    ...
    throw new AppError('Manheim integration request failed', 502, 'INTEGRATION_UPSTREAM_ERROR');
  }
}
